...
public interface APIInterface {
    ...
    @GET("playlistItems?part=snippet&maxResults=15&fields=items(snippet(title,description,thumbnails(high(url),default),resourceId(videoId))),nextPageToken,pageInfo&key=" + Vari.DEVELOPER_KEY)
    Call<VideosResponse> doGetListVideo(@Query("playlistId") String playlistId, @Query("pageToken") String nextPage);



    @GET("playlists?part=snippet,contentDetails&maxResults=15&fields=items(contentDetails,id,snippet(title,description,thumbnails(high(url),default))),nextPageToken,pageInfo&key=" + Vari.DEVELOPER_KEY)
